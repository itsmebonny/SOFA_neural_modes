...
class AnimationStepController(Sofa.Core.Controller):
    def __init__(self, node, *args, **kwargs):
        ...
        # --- New attributes for initial constant force ---
        self.global_simulation_timestep = 0
        self.apply_initial_constant_force = kwargs.get('apply_initial_constant_force', True)
        ...
        self.all_force_magnitudes_newton = [] # To store force magnitudes over time
    ...
    def onAnimateBeginEvent(self, event):
            self.global_simulation_timestep += 1

            # --- Remove previous CFFs at the start of each step ---
            if self.cff_exact is not None:
                try:
                    self.exactSolution.removeObject(self.cff_exact)
                except Exception as e:
                    # print(f"Minor issue removing cff_exact: {e}") # Optional debug
                    pass
                finally:
                    self.cff_exact = None
            ...
            if self.cff_linear is not None:
                try:
                    ...
                    # print(f"Minor issue removing cff_linear: {e}") # Optional debug
                    pass
                finally:
                    self.cff_linear = None
            # --- End Remove CFFs ---

            current_force_to_apply_total = np.array([0.0, 0.0, 0.0])
            apply_cff_this_step = False

            if self.apply_initial_constant_force:
                if self.global_simulation_timestep <= self.initial_force_duration_timesteps:
                    current_force_to_apply_total = self.initial_force_vector_total_np
                    apply_cff_this_step = True
                    
                    if self.global_simulation_timestep == 1: # First step of this phase
                        print(f"\n--- Starting Initial Constant Force Phase (Timesteps 1 to {self.initial_force_duration_timesteps}) ---")
                        # Reset positions to rest at the very beginning of the simulation / force phase
                        print(f"  Resetting positions at timestep 1.")
                    ...
                    print(f"  Timestep {self.global_simulation_timestep}: Applying constant total force: {current_force_to_apply_total}")

                elif self.global_simulation_timestep == self.initial_force_duration_timesteps + 1:
                    # First step after the initial force phase
                    print(f"\n--- Initial Constant Force Phase Ended at timestep {self.initial_force_duration_timesteps}. ---")
                    print(f"--- Timestep {self.global_simulation_timestep}: 'NN takes the wheel' (No explicit CFF from onAnimateBeginEvent). ---")
                    # No force applied by CFF from this event onwards in this mode
                    current_force_to_apply_total = np.array([0.0, 0.0, 0.0]) # Ensure it's zero
                    apply_cff_this_step = False # Explicitly state no CFF
            else:
                # Fallback or alternative force logic if not applying initial constant force
                # This is where the old modal force logic could go if needed as an alternative
                # For now, if not self.apply_initial_constant_force, no forces are set here.
                if self.global_simulation_timestep == 1:
                    print(f"\n--- Initial constant force disabled. No forces applied by onAnimateBeginEvent by default. ---")
                pass


            # Store the magnitude of the force intended for this step
            self.last_applied_force_magnitude_newton = np.linalg.norm(current_force_to_apply_total)
            self.all_force_magnitudes_newton.append(self.last_applied_force_magnitude_newton)

            # --- Apply CFFs if determined above ---
            if apply_cff_this_step:
                try:
                    # Exact Solution
                    self.cff_exact = self.exactSolution.addObject('ConstantForceField',
                                    name="CFF_Exact_InitialPhase",
                                    indices=self.force_roi_indices_exact.tolist(), # Ensure these are valid
                                    totalForce=current_force_to_apply_total.tolist(),
                                    showArrowSize=0.1, showColor="0.9 0.1 0.1 1") # Red for initial force
                    if self.cff_exact: self.cff_exact.init()

                    # Linear Solution
                    self.cff_linear = self.linearSolution.addObject('ConstantForceField',
                                    name="CFF_Linear_InitialPhase",
                                    indices=self.force_roi_indices_linear.tolist(), # Ensure these are valid
                                    totalForce=current_force_to_apply_total.tolist(),
                                    showArrowSize=0.0) 
                    if self.cff_linear: self.cff_linear.init()
                    
                except Exception as e:
                    print(f"ERROR: Failed to create/add/init ConstantForceField(s) for initial force: {e}")
                    traceback.print_exc()
                    if self.root: self.root.animate = False # Stop simulation on CFF error
            # --- End CFF Application ---

            # The old logic for self.modes_to_use, self.current_applied_z, substeps, main_steps,
            # and applying forces based on Phi*z is now bypassed if self.apply_initial_constant_force is true.
            # If you need to run different scenarios *after* the initial force phase,
            # you might re-introduce a simplified main_step logic in the 'else' part of the
            # 'if self.global_simulation_timestep <= self.initial_force_duration_timesteps:' block,
            # potentially to set initial conditions (like initial z) for the "NN takes the wheel" phase.
